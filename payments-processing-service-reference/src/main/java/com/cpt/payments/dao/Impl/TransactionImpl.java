...
public class TransactionImpl implements TransactionDao {
	...
	public Transaction createTransaction(Transaction transaction) {
		...
		try {
			...
			namedJdbcTemplate.update(createTransaction(), new BeanPropertySqlParameterSource(transaction), keyHolder);
	...
	public List<Transaction> fetchAllTransactionsForReconcilation() {
		...
		try {
			transaction = namedJdbcTemplate.query(fetchTransactionsForReconcilation(),
	...
	public void updateRetryCount(Transaction transaction) {
		try {
			...
			namedJdbcTemplate.update(updateRetryCountQuery(), new BeanPropertySqlParameterSource(transaction));
		} catch (Exception e) {
			LogMessage.log(LOGGER, "exception while updating TRANSACTION in DB :: " + transaction);
			LogMessage.logException(LOGGER, e);
		}
